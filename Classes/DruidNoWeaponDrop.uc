...
// Weirdness - looks like sometimes PD called twice, particularly in VINV?
					...
					if (instr(caps(Holder.Weapon.ItemName), "SHIELD GUN") > -1 || instr(caps(Holder.Weapon.ItemName), "ASSAULT RIFLE") > -1 || instr(caps(Holder.Weapon.ItemName), "UTILITY") > -1 || instr(caps(Holder.Weapon.ItemName), "AVRIL") > -1)
					{
						...
						for (OInv=Other.Inventory ; OInv != None && !bok ; OInv=OInv.Inventory)
						{
							...
							if(instr(caps(OInv.ItemName), "ASSAULT RIFLE") > -1)
								AR=OInv;
							if(instr(caps(OInv.ItemName), "UTILITY RIFLE") > -1)
								UTIL=OInv;
							if(instr(caps(OInv.ItemName), "AVRIL") > -1)
								DEKAVRIL=OInv;	
							if (SG != None && AR != None && UTIL != None && DEKAVRIL != None)
								bok = true;
						}
						...
						if (instr(caps(Holder.Weapon.ItemName), "UTILITY RIFLE") > -1 && UTIL != None)
							Other.DeleteInventory(SG);
...
defaultproperties
{
     ...
     LevelCost(3)=40
     AbilityName="Denial"
     Description="The first level of this ability simply prevents you from dropping a weapon when you die (but you don't get it either). The second level allows you to respawn with the weapon and ammo you were using when you died. The third level will save all your weapons when you die. You need to be at least Level 25 to purchase this ability. |This ability does not trigger for self-inflicted death.|Cost (per level): 5,10,40"
     BotChance=1
     MaxLevel=3
}
