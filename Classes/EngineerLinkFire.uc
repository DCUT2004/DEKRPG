...
static function BoostTrap(Projectile P, float SpiderGrowthRate)
{
    local vector NewLoc;
...
static function BoostLaser(DEKLaserGrenadeProjectile Laser, float SpiderGrowthRate)
{
    local vector NewLoc;

    // this code based on the mutator by Rachel 'Angel Mapper' Cordone
    NewLoc = Laser.Location;
    NewLoc.Z += 2*SpiderGrowthRate;
    Laser.SetDrawScale(Laser.DrawScale*SpiderGrowthRate);
    Laser.SetCollisionSize(Laser.CollisionRadius*SpiderGrowthRate,Laser.CollisionHeight*SpiderGrowthRate);
    Laser.SetLocation(NewLoc);
    Laser.DamageRadius *= SpiderGrowthRate;
    Laser.Damage *= SpiderGrowthRate;
	Laser.LaserDamage *= SpiderGrowthRate;
    Laser.MomentumTransfer *= SpiderGrowthRate;
}
...
simulated function ModeTick(float dt)
{
	...
	local BombTrapProjectile BTrap;
	local AerialTrapProjectile ATrap;
	local ShockTrapProjectile STrap;
	local WildfireTrapProjectile WTrap;
	local FrostTrapProjectile FTrap;
	local DEKLaserGrenadeProjectile Laser;
            ...
            else
            {
                ...
                if ( bDoHit )
                {
                    ...
                    if ( !Other.bWorldGeometry )
                    {
						...
						if ( BombTrapProjectile(Other) != None )     // do not affect mines
						{
							BTrap = BombTrapProjectile(Other);
							if (RW_EngineerLink(Instigator.Weapon) != None)
								SpiderBoost = RW_EngineerLink(Instigator.Weapon).SpiderBoost;
							if (BTrap != None && SpiderBoost > 0 && BTrap.Damage < ((1 + SpiderBoost) * BTrap.default.Damage))
							{
							    class'EngineerLinkFire'.static.BoostTrap(BTrap,RW_EngineerLink(Instigator.Weapon).SpiderGrowthRate);
							}
						}
						else
						if ( AerialTrapProjectile(Other) != None )     // do not affect mines
						{
							ATrap = AerialTrapProjectile(Other);
							if (RW_EngineerLink(Instigator.Weapon) != None)
								SpiderBoost = RW_EngineerLink(Instigator.Weapon).SpiderBoost;
							if (ATrap != None && SpiderBoost > 0 && ATrap.Damage < ((1 + SpiderBoost) * ATrap.default.Damage))
							{
							    class'EngineerLinkFire'.static.BoostTrap(ATrap,RW_EngineerLink(Instigator.Weapon).SpiderGrowthRate);
							}
						}
						else
						if ( ShockTrapProjectile(Other) != None )     // do not affect mines
						{
							STrap = ShockTrapProjectile(Other);
							if (RW_EngineerLink(Instigator.Weapon) != None)
								SpiderBoost = RW_EngineerLink(Instigator.Weapon).SpiderBoost;
							if (STrap != None && SpiderBoost > 0 && STrap.Damage < ((1 + SpiderBoost) * STrap.default.Damage))
							{
							    class'EngineerLinkFire'.static.BoostTrap(STrap,RW_EngineerLink(Instigator.Weapon).SpiderGrowthRate);
							}
						}
						else
						if ( WildfireTrapProjectile(Other) != None )     // do not affect mines
						{
							WTrap = WildfireTrapProjectile(Other);
							if (RW_EngineerLink(Instigator.Weapon) != None)
								SpiderBoost = RW_EngineerLink(Instigator.Weapon).SpiderBoost;
							if (WTrap != None && SpiderBoost > 0 && WTrap.Damage < ((1 + SpiderBoost) * WTrap.default.Damage))
							{
							    class'EngineerLinkFire'.static.BoostTrap(WTrap,RW_EngineerLink(Instigator.Weapon).SpiderGrowthRate);
							}
						}
						else
						if ( FrostTrapProjectile(Other) != None )     // do not affect mines
						{
							FTrap = FrostTrapProjectile(Other);
							if (RW_EngineerLink(Instigator.Weapon) != None)
								SpiderBoost = RW_EngineerLink(Instigator.Weapon).SpiderBoost;
							if (FTrap != None && SpiderBoost > 0 && FTrap.Damage < ((1 + SpiderBoost) * FTrap.default.Damage))
							{
							    class'EngineerLinkFire'.static.BoostTrap(FTrap,RW_EngineerLink(Instigator.Weapon).SpiderGrowthRate);
							}
						}
						else
						if ( DEKLaserGrenadeProjectile(Other) != None )     // do not affect mines
						{
							Laser = DEKLaserGrenadeProjectile(Other);
							if (RW_EngineerLink(Instigator.Weapon) != None)
								SpiderBoost = RW_EngineerLink(Instigator.Weapon).SpiderBoost;
							if (Laser != None && SpiderBoost > 0 && Laser.Damage < ((1 + SpiderBoost) * Laser.default.Damage))
							{
							    class'EngineerLinkFire'.static.BoostLaser(Laser,RW_EngineerLink(Instigator.Weapon).SpiderGrowthRate);
							}
						}
				        else
