class RuneStreakProj extends SeekingRocketProj;

var xEmitter RealSmokeTrail;
var Class<xEmitter> RealSmokeTrailClass;
var Actor RealCorona;
var class<Actor> RealCoronaClass;
var Emitter HitEffect;
...
simulated function PostBeginPlay()
{
	...
	RealCorona = Spawn(RealCoronaClass,self);
	if (RealCorona != None)
	{
		RealCorona.SetBase(Self);
		RealCorona.bHardAttach = True;
	}
	Dir = vector(Rotation);
	Velocity = speed * Dir;
    SetTimer(0.1, true);
...
simulated function Timer()
{
	...
	if (Seeking.IsA('HealerNali') || Seeking.IsA('MissionCow'))
		return;
	ForceDir = Normal(Seeking.Location - Location);
	
	VelMag = VSize(Velocity);

	ForceDir = Normal(ForceDir * 1.5 * VelMag + Velocity);
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
    ...
    {
    	HitEffect = Spawn(HitEffectClass,,,HitLocation + HitNormal*20,rotator(HitNormal));
		if (HitEffect != None)
			HitEffect.RemoteRole = ROLE_SimulatedProxy;
    }
	BlowUp(HitLocation);
	Destroy();
}
...
defaultproperties
{
     ...
     MyDamageType=Class'DEKRPG209D.DamTypeRuneStreak'
     LightHue=0
