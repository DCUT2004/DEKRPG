class AbilityMaterialCombatBoots extends AbilityMaterialLowRarity
	config(UT2004RPG)
	abstract;
...
{
	local GroundSpeedMaxMaterialInv Inv;
	...
	if (Other != None)
	{
		//We want to increase the max speed for this player as a bonus.
		//We can do this easily if the player does not have the Quickfoot ability
		//But if they do have Quickfoot, we will spawn an inventory item on the Pawn, tell it to wait a few seconds, and then apply the boost to the max speed
		//This is because after a player ghosts, abilities are called again in the order they are purchased. It is possible that this ability gets called before Quickfoot (which sets a max quickfoot, not increase it, thereby negating this bonus)
		...
		{
			Inv = GroundSpeedMaxMaterialInv(Other.FindInventoryType(Class'GroundSpeedMaxMaterialInv'));
			if (Inv == None)
			{
				Inv = Other.Spawn(Class'GroundSpeedMaxMaterialInv');
		...
		{
			if (Other != None)
				Other.GroundSpeed = Other.default.GroundSpeed * (1 + AbilityLevel*default.LevMultiplier);
		}
	}
}
