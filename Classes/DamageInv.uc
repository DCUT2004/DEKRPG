...
function GiveTo(Pawn Other, optional Pickup Pickup)
{
	...
	NInv = NecroInv(Other.FindInventoryType(class'NecroInv'));
	if(NInv != None)
	{
		return;
	}
...
simulated function Timer()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		if ((DamagePlayerController == None) || (DamagePlayerController.Pawn == None) || (DamagePlayerController.Pawn.Health <= 0) )
		{	// initiating player hasn't got udamage anymore, so artifact must have stopped
			SwitchOffDamage();
			Destroy();
			return;
		}
		if (bSphereActivated && !DamagePlayerController.Pawn.HasUDamage())
		{	// initiating player hasn't got udamage anymore, so artifact must have stopped
			SwitchOffDamage();
			Destroy();
			return;
