...
function bool PreventDeath(Pawn Killed, Controller Killer, class<DamageType> damageType, vector HitLocation)
{
	...
	else if (DEKAutoMachineGunController(Killer) != None)
		PlayerSpawner = DEKAutoMachineGunController(Killer).PlayerSpawner;
	else if (DEKAutoSniperController(Killer) != None)
		PlayerSpawner = DEKAutoSniperController(Killer).PlayerSpawner;
	else if (DEKMercuryController(Killer) != None)
		PlayerSpawner = DEKMercuryController(Killer).PlayerSpawner;
	else if (DEKBeamSentinelController(Killer) != None)
		PlayerSpawner = DEKBeamSentinelController(Killer).PlayerSpawner;
	else if (DEKSniperSentinelController(Killer) != None)
		PlayerSpawner = DEKSniperSentinelController(Killer).PlayerSpawner;
	else if (DEKHellfireSentinelController(Killer) != None)
		PlayerSpawner = DEKHellfireSentinelController(Killer).PlayerSpawner;
	...
	if (DEKFriendlyMonsterController(Killer) != None && Killer.Pawn != None)
	{
		if (!ClassIsChildOf(damageType,class'WeaponDamageType') && !ClassIsChildOf(damageType,class'VehicleDamageType'))
		{
			...
			if ( DEKFriendlyMonsterController(Killer).Master != None && DEKFriendlyMonsterController(Killer).Master.bIsPlayer) 
			{
				// lets see if we can get a different damagetype that will log the pet
				TPRI = TeamPlayerReplicationInfo(DEKFriendlyMonsterController(Killer).Master.PlayerReplicationInfo);
				if ( TPRI != None )
				{
