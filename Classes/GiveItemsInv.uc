...
var bool bVines;
var bool bEmbers;
var bool bArcticSuit;
var bool bMoss;
var bool bDust;
var bool bNanite;
var bool bPumice;
var bool bIcicle;
var bool bTranslator;
var bool bStarChart;
var bool bUranium;
var bool bHourglass;
...
simulated function AddMaterial(Class<AbilityMaterial> MaterialClass)
{
	local Pawn P;
	...
	if (MaterialClass == Class'AbilityMaterialCombatBoots')
	{
		bBoots = True;
		P.ReceiveLocalizedMessage(MessageClass, 1, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialLumber')
	{
		bLumber = True;
		P.ReceiveLocalizedMessage(MessageClass, 2, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialTarydiumShards')
	{
		bTarydium = True;
		P.ReceiveLocalizedMessage(MessageClass, 3, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialSteel')
	{
		bSteel = True;
		P.ReceiveLocalizedMessage(MessageClass, 4, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialNaliFruit')
	{
		bNaliFruit = True;
		P.ReceiveLocalizedMessage(MessageClass, 5, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialGloves')
	{
		bGloves = True;
		P.ReceiveLocalizedMessage(MessageClass, 6, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialLeather')
	{
		bLeather = True;
		P.ReceiveLocalizedMessage(MessageClass, 7, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialPlatedArmor')
	{
		bArmor = True;
		P.ReceiveLocalizedMessage(MessageClass, 8, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialHoneysuckleVine')
	{
		bVines = True;
		P.ReceiveLocalizedMessage(MessageClass, 9, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialEmbers')
	{
		bEmbers = True;
		P.ReceiveLocalizedMessage(MessageClass, 10, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialArcticSuit')
	{
		bArcticSuit = True;
		P.ReceiveLocalizedMessage(MessageClass, 11, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialMoss')
	{
		bMoss = True;
		P.ReceiveLocalizedMessage(MessageClass, 12, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialDust')
	{
		bDust = True;
		P.ReceiveLocalizedMessage(MessageClass, 13, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialNanite')
	{
		bNanite = True;
		P.ReceiveLocalizedMessage(MessageClass, 14, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialPumice')
	{
		bPumice = True;
		P.ReceiveLocalizedMessage(MessageClass, 15, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialIcicle')
	{
		bIcicle = True;
		P.ReceiveLocalizedMessage(MessageClass, 16, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialTranslator')
	{
		bTranslator = True;
		P.ReceiveLocalizedMessage(MessageClass, 17, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialStarChart')
	{
		bStarChart = True;
		P.ReceiveLocalizedMessage(MessageClass, 18, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialUranium')
	{
		bUranium = True;
		P.ReceiveLocalizedMessage(MessageClass, 19, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialHourglass')
	{
		bHourglass = True;
		P.ReceiveLocalizedMessage(MessageClass, 20, None, None, Class);
	}
	else if (MaterialClass == Class'AbilityMaterialMoonlitStone')
	{
		bMoonlitStone = True;
		P.ReceiveLocalizedMessage(MessageClass, 21, None, None, Class);
	}
}

function bool MaterialUnlocked(Class<AbilityMaterial> MaterialClass)
{
	if (MaterialClass == Class'AbilityMaterialCombatBoots' && bBoots)
		return True;
	else if (MaterialClass == Class'AbilityMaterialLumber' && bLumber)
		return True;
	else if (MaterialClass == Class'AbilityMaterialTarydiumShards' && bTarydium)
		return True;
	else if (MaterialClass == Class'AbilityMaterialSteel' && bSteel)
		return True;
	else if (MaterialClass == Class'AbilityMaterialNaliFruit' && bNaliFruit)
		return True;
	else if (MaterialClass == Class'AbilityMaterialGloves' && bGloves)
		return True;
	else if (MaterialClass == Class'AbilityMaterialLeather' && bLeather)
		return True;
	else if (MaterialClass == Class'AbilityMaterialPlatedArmor' && bArmor)
		return True;
	else if (MaterialClass == Class'AbilityMaterialHoneysuckleVine' && bVines)
		return True;
	else if (MaterialClass == Class'AbilityMaterialEmbers' && bEmbers)
		return True;
	else if (MaterialClass == Class'AbilityMaterialArcticSuit' && bArcticSuit)
		return True;
	else if (MaterialClass == Class'AbilityMaterialMoss' && bMoss)
		return True;
	else if (MaterialClass == Class'AbilityMaterialDust' && bDust)
		return True;
	else if (MaterialClass == Class'AbilityMaterialNanite' && bNanite)
		return True;
	else if (MaterialClass == Class'AbilityMaterialPumice' && bPumice)
		return True;
	else if (MaterialClass == Class'AbilityMaterialIcicle' && bIcicle)
		return True;
	else if (MaterialClass == Class'AbilityMaterialTranslator' && bTranslator)
		return True;
	else if (MaterialClass == Class'AbilityMaterialStarChart' && bStarChart)
		return True;
	else if (MaterialClass == Class'AbilityMaterialUranium' && bUranium)
		return True;
	else if (MaterialClass == Class'AbilityMaterialHourglass' && bHourglass)
		return True;
	else if (MaterialClass == Class'AbilityMaterialMoonlitStone' && bMoonlitStone)
		return True;
	...
	for (y = 0; y < StatsInv.DataObject.Abilities.length; y++)
		...
		if (ClassIsChildOf(StatsInv.DataObject.Abilities[y], class'SubClass'))
		{
			...
			if (curSubClassLevel > 0)
			{
				...
				ServerSellData(None, StatsInv, False, False, False);
				return false;
			}
...
function ServerSellAbility(RPGStatsInv StatsInv, int AbilityNo)
{
	...
	ServerSellData(None, StatsInv,False,False,False);	// sell all abilities, as some may have been bought based on the wrong class
}
...
function ServerSellData(PlayerReplicationInfo PRI, RPGStatsInv StatsInv, bool RefundAbility, bool RefundCombo, bool RefundNiche)
{
	local int x;
	...
	// go through the abilities and lose the ones which are not classes
	// Hollon - and in addition to those that are not classes, those that are not materials
	for (x = 0; x < StatsInv.DataObject.Abilities.length; x++)
	{
		if (!ClassIsChildOf(StatsInv.DataObject.Abilities[x], class'RPGClass') && !ClassIsChildOf(StatsInv.DataObject.Abilities[x], class'AbilityMaterial'))	//Exclude selling classes and materials
		{
			if (RefundAbility)		//Refund abilities only. Do not refund combos or niches
			{
				if (!ClassIsChildOf(StatsInv.DataObject.Abilities[x], class'AbilityCombo') && !ClassIsChildOf(StatsInv.DataObject.Abilities[x], class'AbilityNiche'))
				{
					StatsInv.DataObject.Abilities.Remove(x, 1); 
					StatsInv.DataObject.AbilityLevels.Remove(x, 1);
					x--;
			...
			else if (RefundCombo)	//Refund combos only. Do not refund abilities or niches
			{
				if (ClassIsChildOf(StatsInv.DataObject.Abilities[x], class'AbilityCombo'))
				{
					StatsInv.DataObject.Abilities.Remove(x, 1); 
					StatsInv.DataObject.AbilityLevels.Remove(x, 1);
					x--;
			...
			else if (RefundNiche)						//Sell both abilities and combos
			{
				if (ClassIsChildOf(StatsInv.DataObject.Abilities[x], class'AbilityNiche') || StatsInv.DataObject.Abilities[x] == class'AbilityLinkUpBuilder' || StatsInv.DataObject.Abilities[x] == Class'AbilityPrimalEWM')
				{
					StatsInv.DataObject.Abilities.Remove(x, 1); 
					StatsInv.DataObject.AbilityLevels.Remove(x, 1);
					x--;
...
{
	if (Switch == 1)
		return "You discovered a Material!: Combat Boots*" ;
	else if (Switch == 2)
		return "You discovered a Material!: Lumber*" ;
	else if (Switch == 3)
		return "You discovered a Material!: Tarydium Shards*" ;
	else if (Switch == 4)
		return "You discovered a Material!: Steel*" ;
	else if (Switch == 5)
		return "You discovered a Material!: Nali Fruit*" ;
	else if (Switch == 6)
		return "You discovered a Material!: Gloves*" ;
	else if (Switch == 7)
		return "You discovered a Material!: Fine Leather**" ;
	else if (Switch == 8)
		return "You discovered a Material!: Plated Armor**" ;
	else if (Switch == 9)
		return "You discovered a Material!: Honeysuckle Vines**" ;
	else if (Switch == 10)
		return "You discovered a Material!: Burning Embers**" ;
	else if (Switch == 11)
		return "You discovered a Material!: Arctic Suit**" ;
	else if (Switch == 12)
		return "You discovered a Material!: Moss***" ;
	else if (Switch == 13)
		return "You discovered a Material!: Cosmic Dust***" ;
	else if (Switch == 14)
		return "You discovered a Material!: Nanite Fragments***" ;
	else if (Switch == 15)
		return "You discovered a Material!: Pumice***" ;
	else if (Switch == 16)
		return "You discovered a Material!: Icicles***" ;
	else if (Switch == 17)
		return "You discovered a Material!: Universal Translator****" ;
	else if (Switch == 18)
		return "You discovered a Material!: Star Chart****" ;
	else if (Switch == 19)
		return "You discovered a Material!: Uranium Pellets****" ;
	else if (Switch == 20)
		return "You discovered a Material!: Arcane Hourglass****" ;
	else if (Switch == 21)
		return "You discovered a Material!: Moonlit Stone****" ;
