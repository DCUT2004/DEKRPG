...
static function LuckyStrike(Controller Killer, Controller Killed, bool bOwnedByKiller, int AbilityLevel, float Chance)
{
	...
	if (Killed.Pawn != None && Killed.Pawn.IsA('Monster') && !Killed.SameTeamAs(Killer) && AbilityLevel*Chance >= rand(100))
	{
		...
		{
			DropPickups(Killed, Killer, class'DEKRPG208AH.ArtifactLetterOPickup', None, 1);
			return;
		}
		if (ClassIsChildOf(M.Class, class'SMPTitan') || ClassIsChildOf(M.Class, class'SMPStoneTitan'))
		{
			DropPickups(Killed, Killer, class'DEKRPG208AH.ArtifactLetterNPickup', None, 1);
			return;
		}
		if (ClassIsChildOf(M.Class, class'SMPQueen'))
		{
			DropPickups(Killed, Killer, class'DEKRPG208AH.ArtifactLetterUPickup', None, 1);
			return;
		}
		if (ClassIsChildOf(M.Class, class'Warlord'))
		{
			DropPickups(Killed, Killer, class'DEKRPG208AH.ArtifactLetterBPickup', None, 1);
			return;
		}
		if (M.ScoringValue >= default.ScoringValueForS)
		{
			...
			{
				DropPickups(Killed, Killer, class'DEKRPG208AH.ArtifactLetterSPickup', None, 1);
				return;
			}
			else
			{	
				DropPickups(Killed, Killer, class'DEKRPG208AH.GemExperiencePickupPurple', None, 1);
				return;
			}
		}
		else if(M.ScoringValue > 6)
		{
			DropPickups(Killed, Killer, class'DEKRPG208AH.GemExperiencePickupGreen', None, 1);
			return;
		}
		else
		{
			DropPickups(Killed, Killer, class'DEKRPG208AH.GemExperiencePickupBlue', None, 1);			
			return;	
		}
	}
}
