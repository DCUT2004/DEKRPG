...
static function ModifyWeapon(Weapon Weapon, int AbilityLevel)
{
	...
	if (default.BioKills >= default.RequiredKills)
	{
		if (BioFire(FireMode[0]) != None && BioRifle(W) != None)
			BioFire(FireMode[0]).ProjectileClass=class'DEKWeapons209D.UpgradeBioGlob';
		if (BioChargedFire(FireMode[1]) != None && BioRifle(W) != None)
		{
			BioChargedFire(FireMode[1]).MaxGoopLoad=15;
			BioChargedFire(FireMode[1]).ProjectileClass=class'DEKWeapons209D.UpgradeBioGlob';
		}
	}
	if (default.FlakKills >= default.RequiredKills)
	{
		if (FlakFire(FireMode[0]) != None && FlakCannon(W) != None)
		{
			...
			FlakFire(FireMode[0]).ProjectileClass=class'DEKWeapons209D.UpgradeFlakChunk';
		}
		if (FlakAltFire(FireMode[1]) != None && FlakCannon(W) != None)
		{
			FlakAltFire(FireMode[1]).ProjectileClass=class'DEKWeapons209D.UpgradeFlakShell';
		}
	}
	if (default.GrenadeKills >= default.RequiredKills)
	{
		...
		{
			ONSGrenadeFire(FireMode[0]).ProjectileClass=class'DEKWeapons209D.UpgradeGrenadeProjectile';
		}
	}
	if (default.AVRiLKills >= default.RequiredKills)
	{
		...
		{
			INAVRiLFire(FireMode[0]).ProjectileClass=class'DEKWeapons209D.UpgradeINAVRiLRocket';
		}
	}
	...
	if (default.RedeemerKills >= default.RequiredKills)
	{
		...
		{
			RedeemerFire(FireMode[0]).ProjectileClass=class'DEKWeapons209D.UpgradeRedeemerProj';
		}
	}
...
defaultproperties
{
     ...
     L9MaxMines=12
     L10MaxMines=13
     ExcludingAbilities(0)=Class'DEKRPG209D.AbilitySpecialistProficiency'
     ExcludingAbilities(1)=Class'DEKRPG209D.AbilityDualityProficiency'
