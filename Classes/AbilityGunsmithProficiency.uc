...
static function ModifyWeapon(Weapon Weapon, int AbilityLevel)
{
	...
	if (default.BioKills >= default.RequiredKills)
	{
		if (BioFire(FireMode[0]) != None && BioRifle(W) != None)
			BioFire(FireMode[0]).ProjectileClass=class'DEKWeapons208AE.UpgradeBioGlob';
		if (BioChargedFire(FireMode[1]) != None && BioRifle(W) != None)
		{
			BioChargedFire(FireMode[1]).MaxGoopLoad=15;
			BioChargedFire(FireMode[1]).ProjectileClass=class'DEKWeapons208AE.UpgradeBioGlob';
		}
	}
	...
	if (default.GrenadeKills >= default.RequiredKills)
	{
		...
		{
			ONSGrenadeFire(FireMode[0]).ProjectileClass=class'DEKWeapons208AE.UpgradeGrenadeProjectile';
		}
	}
	if (default.AVRiLKills >= default.RequiredKills)
	{
		...
		{
			INAVRiLFire(FireMode[0]).ProjectileClass=class'DEKWeapons208AE.UpgradeINAVRiLRocket';
		}
	}
	...
	if (default.RedeemerKills >= default.RequiredKills)
	{
		...
		{
			RedeemerFire(FireMode[0]).ProjectileClass=class'DEKWeapons208AE.UpgradeRedeemerProj';
		}
	}
	...
	if (default.ShockKills >= default.RequiredKills)
	{
		...
		{
			ShockProjFire(FireMode[1]).ProjectileClass=Class'DEKWeapons208AE.UpgradeShockProjectile';
		}
	}
}
...
defaultproperties
{
     ...
     L9MaxMines=12
     L10MaxMines=13
     ExcludingAbilities(0)=Class'DEKRPG208AE.AbilitySpecialistProficiency'
     ExcludingAbilities(1)=Class'DEKRPG208AE.AbilityDualityProficiency'
