...
static function ModifyWeapon(Weapon Weapon, int AbilityLevel)
{
	...
	if (default.BioKills >= default.RequiredKills)
	{
		if (BioFire(FireMode[0]) != None && BioRifle(W) != None)
			BioFire(FireMode[0]).ProjectileClass=class'DEKWeapons208AD.UpgradeBioGlob';
		if (BioChargedFire(FireMode[1]) != None && BioRifle(W) != None)
		{
			BioChargedFire(FireMode[1]).MaxGoopLoad=15;
			BioChargedFire(FireMode[1]).ProjectileClass=class'DEKWeapons208AD.UpgradeBioGlob';
		}
	}
	...
	if (default.GrenadeKills >= default.RequiredKills)
	{
		...
		{
			ONSGrenadeFire(FireMode[0]).ProjectileClass=class'DEKWeapons208AD.UpgradeGrenadeProjectile';
		}
	}
	if (default.AVRiLKills >= default.RequiredKills)
	{
		...
		{
			INAVRiLFire(FireMode[0]).ProjectileClass=class'DEKWeapons208AD.UpgradeINAVRiLRocket';
		}
	}
	...
	if (default.ShockKills >= default.RequiredKills)
	{
		...
		{
			ShockProjFire(FireMode[1]).ProjectileClass=Class'DEKWeapons208AD.UpgradeShockProjectile';
		}
	}
}
...
defaultproperties
{
     ...
     ExcludingAbilities(1)=Class'DEKRPG208AD.AbilityDualityProficiency'
