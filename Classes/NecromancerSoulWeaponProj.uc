class NecromancerSoulWeaponProj extends Projectile;

var NecromancerSoulWeaponProjTrail FX;

simulated function PostBeginPlay()
{
	Velocity = Speed * vector(Rotation);
	FX = spawn(class'NecromancerSoulWeaponProjTrail');
	if (FX != None)
	{
		FX.SetBase(Self);
		FX.Lifespan = 2.00;
...
simulated function SpawnEffects( vector HitLocation, vector HitNormal )
{
	...
	if ( EffectIsRelevant(Location,false) )
	{
		...
		//if ( (PC.ViewTarget != None) && VSize(PC.ViewTarget.Location - Location) < 3000 )
			//spawn(class'DEKRPG208AE.NecromancerBloodWeaponExplosion',,,HitLocation + HitNormal*16 );
...
defaultproperties
{
     ...
     MyDamageType=Class'DEKRPG208AE.DamTypeNecromancerSoulWeapon'
     ...
     Skins(1)=Texture'EpicParticles.Flares.Sharpstreaks2'
     SoundVolume=50
