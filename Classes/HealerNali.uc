...
function RangedAttack(Actor A)
{
	...
	else if ( VSize(A.Location - Location) <= CloseRangeDistance + CollisionRadius + A.CollisionRadius )
	{
		...
		if (rand(99) <= CloseRareEventChance && P != None)
		{
			...
			{
				if (rand(99) <= 20)
					DropPickups(Self.Controller, P.Controller, class'DEKRPG999X.ArtifactLetterBPickup', None, 1);
				else if (rand(99) <= 40)
					DropPickups(Self.Controller, P.Controller, class'DEKRPG999X.ArtifactLetterOPickup', None, 1);
				else if (rand(99) >= 60)
					DropPickups(Self.Controller, P.Controller, class'DEKRPG999X.ArtifactLetterNPickup', None, 1);
				else if (rand(99) <= 80)
					DropPickups(Self.Controller, P.Controller, class'DEKRPG999X.ArtifactLetterUPickup', None, 1);
				else if (rand(99) <= 100)
					DropPickups(Self.Controller, P.Controller, class'DEKRPG999X.ArtifactLetterSPickup', None, 1);
			}
			else
				DropPickups(Self.Controller, P.Controller, class'DEKRPG999X.GemExperiencePickupBlue', None, 1);
		}
		else
			AddHealth(HealthDamage);
	}
...
defaultproperties
{
     ...
     ScoringValue=0
     Health=30
     ControllerClass=Class'DEKRPG999X.HealerNaliController'
}
