class FlowerYellowPickup extends TournamentPickup;
...
auto state Pickup
{
	function Touch(Actor Other)
	{
		...
		if (ValidTouch(Other))
		{
			...
			{
				Inv = FlowerShieldInv(P.FindInventoryType(class'FlowerShieldInv'));
				if (Inv == None)
				{
					Inv = P.Spawn(class'FlowerShieldInv');
					Inv.GiveTo(P);
					AnnouncePickup(P);
					Destroy();
				}
...
defaultproperties
{
     ...
     DrawScale=0.255000
     Skins(0)=Texture'DEKRPGTexturesMaster209A.EarthFlowers.YellowFlowerLeaf0'
     Skins(1)=Texture'DEKRPGTexturesMaster209A.EarthFlowers.OrangeFlowerCenter1'
     Skins(2)=Texture'DEKRPGTexturesMaster209A.EarthFlowers.OrangeFlowerBottom2and3'
     Skins(3)=Texture'DEKRPGTexturesMaster209A.EarthFlowers.OrangeFlowerBottom2and3'
     AmbientGlow=255
     ScaleGlow=0.500000
     Style=STY_AlphaZ
