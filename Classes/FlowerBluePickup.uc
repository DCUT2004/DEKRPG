class FlowerBluePickup extends TournamentPickup;
...
auto state Pickup
{
	function Touch(Actor Other)
	{
		...
		if (ValidTouch(Other))
		{
			...
			{
				Inv = FlowerHealInv(P.FindInventoryType(class'FlowerHealInv'));
				if (Inv == None)
				{
					Inv = P.Spawn(class'FlowerHealInv');
					Inv.GiveTo(P);
					AnnouncePickup(P);
					Destroy();
				}
...
defaultproperties
{
     ...
     DrawScale=0.255000
     Skins(0)=Texture'DEKRPGTexturesMaster208K.EarthFlowers.BlueFlowerLeaf0'
     Skins(1)=Texture'DEKRPGTexturesMaster208K.EarthFlowers.BlueFlowerCenter1'
     Skins(2)=Texture'DEKRPGTexturesMaster208K.EarthFlowers.BlueFlowerBottom2and3'
     Skins(3)=Texture'DEKRPGTexturesMaster208K.EarthFlowers.BlueFlowerBottom2and3'
     AmbientGlow=255
     ScaleGlow=0.500000
     Style=STY_AlphaZ
